...
shinyServer(function(input,output,session){
  ...
  output$de_sf<-renderUI({
    if(input$seurat_de_action1==0){
      return()
    }
    withProgress(message = 'Feature Plot...', value = 0,{
    marker_display_option()
    s_markers<<-pb.markers
    genes1<-s_markers$gene
    selectInput("feature_plot","Select Genes for Plotting",choice=genes1)})
  })
output$feature_plot_result<-renderPlot({
  ...
  data <- reactive({
    ...
    if (input$testme) {
      ...
      inputfile <- read.csv("Combined_expression.csv", header= TRUE, sep=',',row.names = as.numeric(TRUE),stringsAsFactors = FALSE)
    }
    else if(! is.null(input$File100)){
      req(input$File100)
      req(input$File101)
      req(input$File102)
      File100 <- input$File100
      File101 <- input$File101
      File102<- input$File102
      file_x<-rbind(File100,File101,File102)
      
      #file_list<-list(File100,File101,File102)
      filesdir =dirname(file_x[1,4])
      
      file_x$name[1]<-"barcodes.tsv"
      file_x$name[2]<-"genes.tsv"
      file_x$name[3]<-"matrix.mtx"
      
      file.rename(file_x$datapath[1],paste0(filesdir,'/',file_x$name[1]))
      file.rename(file_x$datapath[2],paste0(filesdir,'/',file_x$name[2]))
      file.rename(file_x$datapath[3],paste0(filesdir,'/',file_x$name[3]))
  ...
  barcode_table<- reactive({
    b1<-read.table("barcodes.tsv",header = TRUE)
    b1
  })
  genes_table<- reactive({
    b2<-read.table("genes.tsv",header = TRUE)
    b2
  })
  matrix_table<-reactive({
    b3<-readMM("matrix.mtx")
    b3
  })
  
  output$barcode <- downloadHandler(
    filename = function() {
      paste("barcodes.tsv")
    },
    content = function(file) {
      write.table(barcode_table(), file,row.names = FALSE)
  ...
  output$genes <- downloadHandler(
    ...
    content = function(file) {
      write.table(genes_table(), file,row.names = FALSE)
  ...
  output$matrix <- downloadHandler(
    ...
    content = function(file) {
      writeMM(matrix_table(), file)
      
    })
data1<-reactive({
  if (input$testme3) {
    #write.csv(meta,"meta.csv",row.names = FALSE)
    inputfile <- read.csv("Combined_metadata.csv", header= TRUE, sep=',',row.names = as.numeric(TRUE),stringsAsFactors = FALSE)
  }
  ...
  meta1<-input$meta
inputfile<- read.csv(file=meta1$datapath,sep=",", header= TRUE,row.names = as.numeric(TRUE),stringsAsFactors = FALSE)}
...
output$dirp<-renderPrint({
  ...
  observeEvent(input$mast2, {
    # Show a modal when the button is pressed
    
    shinyalert(animation = TRUE,
               imageUrl = "loader_mast.gif",
               imageWidth = 400,
               imageHeight = 350,confirmButtonText="OK",confirmButtonCol="black")
    #src = "gig.gif"
  })
...
output$meta_info_dropclust<-renderUI({
  ...
  v1$Sample<-zx
  v1<-as.data.frame(v1)
  v2<<-as.data.frame(meta_d)
  v2$Sample<-rownames(v2)
  
  v3<-merge(v2,v1,by.x="Sample",by.y="Sample")
  
  
  colData(sce_m)<<- DataFrame(v3)
  choices_meta_d<<-colnames(sce_m@colData)
  
  selectInput("meta_header_d","Select Parameter for Batch Correction",choices = choices_meta_d,selected=choices_meta_d[2])
})



output$meta_info_mast<-renderUI({
  if(input$mast1==0){
    return()
  }
  meta_d<<-data1()
 ...
 colData(sce)<<- DataFrame(v3)
  choices_meta_d<<-colnames(sce@colData)
  
  selectInput("meta_header_mast","Select Condition for DEG Analysis",choices = choices_meta_d,selected=choices_meta_d[2])
})
output$condition_info<-renderTable({
  if(input$mast2==0){
    return()
  }
  withProgress(message = 'Marker Identification...', value = 0,{
    freq_expressed <- input$m1
    FCTHRESHOLD <- log2(input$m2)
    cdr2 <-colSums(assay(sce)>0)
    colData(sce)$cngeneson <- scale(cdr2)
    x<-colData(sce)
    x<-as.data.frame(x)
    x<-x[,input$meta_header_mast]
    
    cond<<-factor(x)
    cond<-relevel(cond,input$condition1)
    colData(sce)$condition<-cond
    sce22<-SceToSingleCellAssay(sce)
    
    thres <- thresholdSCRNACountMatrix(assay(sce22), nbins = 20, min_per_bin = 30)
    
    thresh=thres$counts_threshold

    freq_expressed <- input$m1
    FCTHRESHOLD <- log2(input$m2)
    expressed_genes <- freq(sce22) > freq_expressed
    sce22 <<- sce22[expressed_genes,]
    
    
    zlmCond <- MAST::zlm(~condition + cngeneson, sce22, parallel = TRUE)
    
    #zlmCond <- zlm(~condition + cngeneson, sce22)
    summaryCond <- summary(zlmCond, doLRT=input$condition2)
    summaryDt <- summaryCond$datatable
    #colnames(summaryDt)[which(names(summaryDt) == "Pr(>Chisq)")] <- "chisq"
    c3<-subset(summaryDt,component=='H'& contrast==input$condition2)
    c3<-c3[,c("primerid","Pr(>Chisq)")]
    c4<-subset(summaryDt,component=='logFC'& contrast==input$condition2)
    c4<-c4[,c("primerid","coef", "ci.hi", "ci.lo")]
    fcHurdle <-merge(c3,c4,by.x="primerid",by.y="primerid")#logFC coefficients
    
    fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
    fcHurdleSig <<- merge(fcHurdle[fdr<input$m3 & abs(coef)>FCTHRESHOLD], as.data.table(mcols(sce22)), by='primerid')
    setorder(fcHurdleSig, fdr)
    head(fcHurdleSig,20)
  }) 
})

output$meta_info_condition<-renderUI({
  if(input$mast1==0){
    return()
  }
  withProgress(message = 'Parameter Selection...', value = 0,{
    
    x<-colData(sce)
    x<-as.data.frame(x)
    x<-x[,input$meta_header_mast]
    y<-unique(x)
    selectInput("condition1","Select Reference Level",choices=y)
    
  }) 
})
output$meta_info_condition1<-renderUI({
  if(input$mast1==0){
    return()
  }
  withProgress(message = 'Parameter Selection...', value = 0,{
    ...
    x<-x[,input$meta_header_mast]
    y<-unique(x)
    z<-paste("condition",sep="",unique(x))
    selectInput("condition2","Select Parameter for Coefficient Hypothesis ",choices=z,selected = z[2])
...
qq_scater<-reactive({
  ...
  cells_before<<-length(colnames(x))
  
  x[["percent.mt"]] <<- PercentageFeatureSet(x, pattern = "^MT-")
  x[["percent.ERCC"]] <<- PercentageFeatureSet(x, pattern = "^ERCC-")
  ...
  if(input$ERCC2){
    if(x[["percent.ERCC"]]==0){
      ...
      plot1 <- FeatureScatter(x, feature1 = "nCount_RNA", feature2 = "percent.ERCC")
      x <<- subset(x, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna & percent.ERCC <= input$ercc_rna)
      p4<-VlnPlot(x, features = c("percent.ERCC"))+ scale_y_continuous(limits = c(0,100)) 
      plot_grid(p4,plot1)
    }   
    else{
      x[["percent.ERCC"]] <<- PercentageFeatureSet(x, pattern = "^ERCC-")
      ...
      plot1 <- FeatureScatter(x, feature1 = "nCount_RNA", feature2 = "percent.ERCC")
      x <<- subset(x, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna & percent.ERCC <= input$ercc_rna)
      p_box<-VlnPlot(x, features = c("percent.ERCC")) 
      plot_grid(p4,plot1)
    } 
  }
    ...
    if(x[["percent.mt"]]==0){
      x[["percent.mt"]] <<- PercentageFeatureSet(x, pattern = "^MT-")
      x <<- subset(x, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna )
      plot1 <- FeatureScatter(x, feature1 = "nCount_RNA", feature2 = "percent.mt")
      plot1 
      p4<-VlnPlot(x, features = c("percent.mt"))+ scale_y_continuous(limits = c(0,100)) 
      plot_grid(p4,plot1)}
    else{
      x[["percent.mt"]] <<- PercentageFeatureSet(x, pattern = "^MT-")
      x <<- subset(x, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna )
...
output$qc_scater_download <- downloadHandler(
  filename = function(){'QC_plot.pdf'},
  content = function(file){
    pdf(file)
    print(    qq_scater())
...
qq_scater1<-reactive({
  ...
  cells_after<-length(colnames(sce))
  x<-c(cells_before,cells_after)
  x<-as.data.frame(x)
  
  x$condition<-c("Cells Before QC","Cells After QC")
  names(x)<-c("Number of Cells","condition")
  par(mfrow=c(1,1))
  ggplot(data=x, aes(x=condition, y=`Number of Cells`)) +
    geom_bar(stat="identity", fill="steelblue")+
    geom_text(aes(label=`Number of Cells`), vjust=1.6, color="white", size=3.5)+
    theme_minimal()+ scale_x_discrete(limits=c("Cells Before QC","Cells After QC" ))
  
})

output$qc_scater_download1 <- downloadHandler(
  filename = function(){'Bar_plot.pdf'},
  content = function(file){
    pdf(file)
    print(qq_scater1())
...
output$downloadData_mast <- downloadHandler(
  filename = function() {
    paste("data-", Sys.Date(), ".csv", sep="")
  },
  content = function(file) {
    write.csv(fcHurdleSig, file)
 ...
 meta_d<-data1()
  ...
  output$seu_object <- renderPrint({
    ...
    withProgress(message = 'Creating Object...', value = 0,{
      if(input$low_seq_depth)
        
      
      {
        pb <<- CreateSeuratObject(counts = data(), min.cells = input$min_cell, min.features = input$min_features,project = "scRNAseq")
        meta_d<<-data1()
        pb<<-AddMetaData(pb,meta_d)  
      pb<<-subset(pb, Mapped_reads> input$min_seqdepth)
      print(pb)
  ...
  output$featurescatter_explanation<-renderPrint({
    if(input$seurat1==0){
      return()
    }
    paste("Scatter plot representing the relationship between indicated features of the cells")
  })
  ...
  output$q1_explanation<-renderPrint({
    if(input$SCE1==0){
      return()
    }
    paste("QC metrics is for the identification of low-quality cells based on their expression profiles")
  })
  output$q2_explanation<-renderPrint({
    if(input$SCE1==0){
      return()
    }
    paste("Bar graph representing the number of cells before (left) and after (right) Quality Control filtering")
  })
  ...
  output$dsce_silhouette<-renderPrint({
    ...
    paste("Silhouette plot checks the separateness of the clusters using the silhouette width. Cells with large positive silhouette widths are closer to other cells in the same cluster than to cells in different clusters. Conversely, cells with negative widths are closer to other clusters than to other cells within their own cluster. Each cluster would ideally contain several cells with large positive widths, indicating that it is well-separated from other clusters")
  })
  ...
  vln<-reactive({
     if(input$ERCC){
      pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
      pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
      if(pb[["percent.mt"]]==0&pb[["percent.ERCC"]]==0){
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        p1<- VlnPlot(pb, features = c("nFeature_RNA"))
        p2<-VlnPlot(pb, features = c("nCount_RNA"))
        p3<-VlnPlot(pb, features = c("percent.mt"))+ scale_y_continuous(limits = c(0,100))
        p4<-VlnPlot(pb, features = c("percent.ERCC"))+ scale_y_continuous(limits = c(0,100))
        plot_grid(p1,p2,p3,p4)
      }
      else if(pb[["percent.mt"]]==0){
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        ...
        p2<-VlnPlot(pb, features = c("nCount_RNA"))
        p3<-VlnPlot(pb, features = c("percent.mt"))+ scale_y_continuous(limits = c(0,100))
        p4<-VlnPlot(pb, features = c("percent.ERCC"))
        plot_grid(p1,p2,p3,p4)
      }
      else if(pb[["percent.ERCC"]] ==0){
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        pb[["percent.ERCC"]] <<- PercentageFeatureSet(pb, pattern = "^ERCC-")
        pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
        p1<- VlnPlot(pb, features = c("nFeature_RNA"))
        p2<-VlnPlot(pb, features = c("nCount_RNA"))
        p3<-VlnPlot(pb, features = c("percent.mt"))
        p4<-VlnPlot(pb, features = c("percent.ERCC"))+ scale_y_continuous(limits = c(0,100))
        plot_grid(p1,p2,p3,p4)
      }
      else{
      ...
      pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
      if(pb[["percent.mt"]]==0){
        VlnPlot(pb, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) +scale_y_continuous(limits = c(0,100))
      }
      else{
  ...
  output$featurescatter_plot<-renderPlot({
    if(input$seurat1==0){
      return()
    }
    pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
    plot1 <- FeatureScatter(pb, feature1 = "nCount_RNA", feature2 = "percent.mt")
  ...
  output$ex_variable<-renderPrint({
    ...
    paste("Visualization of QC metrics as violin plots after filtering the cells based on number of genes (nFeature_RNA) and reads (nCount_RNA)")
  })
  ...
  featurescatterPlot1<-reactive({
    plot1 <- FeatureScatter(pb, feature1 = "nCount_RNA", feature2 = "percent.mt")
    plot2 <- FeatureScatter(pb, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
    plot1
  })
  
  output$featurescatterPlot <- downloadHandler(
    filename = function(){'VLN_plot.pdf'},
    content = function(file){
      pdf(file)
      print(    featurescatterPlot1()
      )
      dev.off()
    })
  ...
  output$qc_seurat<-renderPrint({
    if(input$seurat1==0){
      return()
    }
    withProgress(message = 'Cells after QC...', value = 0,{
      pb <<- subset(pb, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna)
     length(colnames(pb))
      })
  })
  ...
  variableplot<-reactive({
    ...
    pb <<- FindVariableFeatures(pb, selection.method = "vst", nfeatures = input$find_var_feature)
  ...
  output$plot_vf<-renderPlot({
    ...
    withProgress(message = 'Creating Variable Feature Plot...', value = 0,{
      if(input$ERCC)
      {
        ...
        pb <<- subset(pb, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna &percent.ERCC<=input$ercc_rna)  
        pb <<- NormalizeData(pb)
...
cca_pca<-reactive({
if(input$vars_regress)
   {
  pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
  
  all.genes <- rownames(pb)
  pb <<- ScaleData(pb, features = all.genes,vars.to.regress = c("nCount_RNA","percent.mt"))
      ...
      withProgress(message = 'Generating PCA Plot...', value = 0,
                   {
                     if(input$vars_regress)
                     {
                       ...
                       all.genes <- rownames(pb)
                       pb <<- ScaleData(pb, features = all.genes)
                       pb <<- RunPCA(pb, features = VariableFeatures(object = pb),npcs = 30)
                       DimPlot(pb, reduction = "pca", pt.size = 4, group.by = "ident")}
                   })
  })
...
paste("The goal of this algorithm is to learn the underlying manifold of the data in order to place similar cells together in low-dimensional space. Cells within the graph-based clusters determined above should co-localize on these dimensionality reduction plots")
  ...
  tissue_tab_d<-reactive({
    ...
    output$umap<-renderPlot({
      ...
      identity_stouffer<<-Idents(pb)
      
      sce<<-as.SingleCellExperiment(pb)
      if(length(rowData(sce)$featureType=="mito")!=0)
      {

        sce <- addPerCellQC(sce, subsets=list(Mt=rowData(sce)$featureType=="mito"))

        qc <<- quickPerCellQC(colData(sce),
        ...
        colData(sce)$sum<-colData(sce)$nCount_RNA
        colData(sce)$detected<-colData(sce)$nFeature_RNA
        colData(sce)$subsets_Mt_sum<- 0
        colData(sce)$subsets_Mt_detected<-0
        colData(sce)$subsets_Mt_percent<-0
        qc <<- quickPerCellQC(colData(sce),
                              sub.fields="subsets_Mt_percent")
        sce$discard<- qc$discard
      }
      ...
      chosen.hvgs <<- getTopHVGs(dec, n=input$hvg)
      ...
      withProgress(message = 'Generating Stouffer Plot...', value = 0,
       ...
       # Stouffer_CellEnrich()
        ...
        if(length(same_genes)>1){
          ...
          cell_types<<-identity_stouffer[names(stouffer_score)]
    ...
    output$de_karyotype<-renderPlot({
      ...
      withProgress(message = 'Generating Karyoplot...', value = 0,{
...
output$SCE_plot<-renderPlot({
  ...
  withProgress(message = 'Creating SingleCellExperiment Object and Performing Quality Control...', value = 0,
               {
                  x <<- CreateSeuratObject(counts = data(), min.cells = 3, min.features = 200,project = "scRNAseq")
                  cells_before<<-length(colnames(x))
                   ...
                   x[["percent.ERCC"]] <<- PercentageFeatureSet(x, pattern = "^ERCC-")
                    x[["percent.mt"]] <<- PercentageFeatureSet(x, pattern = "^MT-")
                     ...
                     plot_grid(p4,plot1)
                      ...
                      plot1 <- FeatureScatter(x, feature1 = "nCount_RNA", feature2 = "percent.ERCC")
                      x <<- subset(x, subset = nFeature_RNA > input$nFeature_rna & nFeature_RNA < input$count_rna & percent.mt < input$mito_rna & percent.ERCC <= input$ercc_rna)
                      p4<-VlnPlot(x, features = c("percent.ERCC")) 
                      plot_grid(p4,plot1)
                    } 
                  }
...
output$cells_qc<-renderPrint({
  if(input$SCE1==0){
    return()
  }
 b<- length(colnames(sce))
 b
})
output$SCE_tsneplot<-renderPlot({
  if(input$SCE1==0){
    return()
  }
  withProgress(message = 'Dimensionality Reduction...', value = 0,
               {   
       if(input$low_seq_depth1){
         ...
         colData(sce_j)<<- DataFrame(v3) 
        sce_j<<-subset(sce_j, colData(sce_j)$Mapped_reads> input$min_seqdepth1)
         sce<<-sce
         sce <<- computeSumFactors(sce)
         summary(sizeFactors(sce))
  ...
  withProgress(message = 'Clustering Using t-SNE...', value = 0,
               {
                 ...
                 my.clusters <<- unname(cutreeDynamic(my.tree, distM=as.matrix(my.dist), verbose=0))
                 sce$cluster <<- factor(my.clusters)
                 matric<<-sce@assays@data$logcounts
                 names(sce$cluster)<-colnames(sce)
                 identity_stouffer<<-sce$cluster
...
#     pb <<- NormalizeData(pb, normalization.method = "LogNormalize", scale.factor = 10000)
#     pb <<- FindVariableFeatures(pb, selection.method = "vst", nfeatures = 2000)
#     
...
output$sce_silhouette_d1<- downloadHandler(
  filename = function(){'plot.pdf'},
  content = function(file){
    pdf(file)
...
output$de_f<-renderUI({
  if(input$scater_de_action1==0){
    return()
  }
  genes1<-pb.markers$gene
  selectInput("feature_plot_sce","Select Genes for Plotting",choice=genes1,selected = genes1[1])
})
  ...
  withProgress(message = 'Batch Correction...', value = 0,
               {
                # sce<<-SingleCellExperiment(as.matrix(data()))
                 ...
                 sce1<<-batchelor::fastMNN(sce_m,  batch=colData(sce_m)[,input$meta_header_d]
...
output$harm2<-renderPrint({
  ...
  withProgress(message = 'Displaying Harmony Embeddings...', value = 0,{
    sce<<-as.SingleCellExperiment(pb)
    if(length(grep("mt-", rownames(sce)))!=0)
    {
...
harm3_p<-reactive({
  ...
  pb %>%
    RunHarmony(input$meta_header, plot_convergence = TRUE)
  p1 <- DimPlot(object = pb, reduction = "harmony", pt.size = .5, group.by = input$meta_header)
  p2 <- VlnPlot(object = pb, features = "harmony_1", pt.size = .5,group.by = input$meta_header)
  plot_grid(p1,p2)
})
...
output$harm4<-renderPlot({
  ...
  umap_mat<<-pb[["umap"]]@cell.embeddings
  umap_mat<<-as.data.frame(umap_mat)
  DimPlot(pb, reduction = "umap",pt.size = 4, group.by = input$meta_header,)
