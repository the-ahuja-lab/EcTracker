...
qq_scater1<-reactive({
  ...
  y<-c("Cells before QC","Cells after QC")
  par(mfrow=c(1,1))
  barplot(x,main="Cell Filteration",
          xlab="Cells",
          ylab="Count",
          col="cyan")
})
...
metadata_add<-reactive({
 meta_d<-data1()
  ...
  output$featurescatter_explanation<-renderPrint({
    ...
    paste("Scatter plot representing the relationship between two features of the cells")
  })
  ...
  output$q1_explanation<-renderPrint({
    ...
    paste("QC metrics is for identification of low-quality cells based on their expression profiles")
  })
  ...
  vln<-reactive({
    if(input$ERCC){
      ...
      VlnPlot(pb, features = c("nFeature_RNA", "nCount_RNA", "percent.mt","percent.ERCC"), ncol = 4)+ scale_y_continuous(limits = c(0,Inf))
    }
    
    else{
      pb[["percent.mt"]] <<- PercentageFeatureSet(pb, pattern = "^MT-")
      VlnPlot(pb, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) + scale_y_continuous(limits = c(0,Inf))
    } 
  ...
  withProgress(message = 'Creating SingleCellExperiment Object and Performing Quality Control...', value = 0,
               {
                 ...
                 if(input$ERCC2){
                   if(length(grep("ercc-", rownames(sce)))!=0){
                     sce <- addPerCellQC(sce, subsets=list(ercc=grep("ercc-", rownames(sce))))
                     if(length(grep("mt-", rownames(sce)))!=0)
                     {
                       ...
                       plotColData(sce, x="sum", y="subsets_ercc_percent", colour_by="discard")
...
harm3_p<-reactive({
  p1 <- DimPlot(object = pb, reduction = "harmony", pt.size = .3, group.by = input$meta_header)
  p2 <- VlnPlot(object = pb, features = "harmony_1", pt.size = .3,group.by = input$meta_header)
  plot_grid(p1,p2)
})
...
output$harmony_plot3 <- downloadHandler(
 ...
 output$meta_info2<-renderUI({
  ...
  # AddMetaData(pancreas,meta.data = data1())
   ...
   pb<<-AddMetaData(pb,meta_d)
   choices_meta<<-colnames(pb@meta.data)
   selectInput("meta_header","Select Parameter for Batch Correction",choices = choices_meta[7])
 })
