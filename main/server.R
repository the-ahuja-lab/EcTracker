...
output$condition_info<-renderTable({
  ...
  withProgress(message = 'Marker Identification...', value = 0,{
    ...
    #assays(sce22, withDimnames = FALSE) <- list(thresh=thres$counts_threshold, tpm=assay(sce22))
...
output$SCE_markers<-renderPrint({
  ...
  withProgress(message = 'Marker Identification...', value = 0,{
    ...
    pb <<- FindClusters(pb, resolution = 0.5)
    pb <<- RunUMAP(pb, dims = 1:10)
    umap_mat<<-pb[["TSNE"]]@cell.embeddings
...
scatter_drop<-reactive({
  ...
  sce <<- runPCA(sce, subset_row=chosen.hvgs)
  sce <<- runTSNE(sce, dimred="PCA")
  plotReducedDim(sce, dimred = "PCA", colour_by = "level1class")
})
