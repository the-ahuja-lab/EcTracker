  ...
  withProgress(message = 'Creating SingleCellExperiment Object and Performing Quality Control...', value = 0,
               {
                 ...
                 sce <<- as.SingleCellExperiment(x)
                 if(input$ERCC2){
                   if(length(grep("ERCC-", rownames(sce)))!=0){
                     is.spike <- grepl("^ERCC", rownames(sce))
                     sce <- addPerCellQC(sce, list(ercc= is.spike))
                      ...
                      if(length(grep("mt-", rownames(sce)))!=0)
                     {
                       ...
                       qc <<- quickPerCellQC(colData(sce),
                                             sub.fields=c("subsets_Mt_percent","subsets_ercc_percent"))
                       sce$discard <- qc$discard
                       sce$Count  <-sce$sum
                       plotColData(sce, x="Count", y="subsets_ercc_percent", colour_by="discard")+scale_y_continuous(limits = c(0,100))
                     ...
                     if(length(grep("mt-", rownames(sce)))!=0)
                     {
                       ...
                       sce <- addPerCellQC(sce, subsets=list(Mito=grep("mt-", rownames(sce))))
                       colData(sce)$subsets_ercc_sum<- 0
                       colData(sce)$subsets_ercc_detected<-0
                       colData(sce)$subsets_ercc_percent<-0
                       qc <<- quickPerCellQC(colData(sce), 
                                             sub.fields="subsets_Mt_percent")
                       sce$discard <- qc$discard
                     sce$Count  <-sce$sum
                 ...
                 if(length(grep("mt-", rownames(sce)))!=0)
                 {
                   ...
                   sce$discard <- qc$discard
                   sce$Count  <-sce$sum
                   plotColData(sce, x="Count", y="subsets_Mt_percent", colour_by="discard")+scale_y_continuous(limits = c(0,100))
